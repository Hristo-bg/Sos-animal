window.onload = () => {
  ...
  console.log('SOS Animal - Initializing...');
  const els = {
  ...
  const API_BASE = '/api';
  const MYSQL_OFFLINE_MSG = 'Включете XAMPP MySQL!';
  ...
  async function checkDatabaseHealth() {
    try {
      ...
      const dbDown = data.database === 'disconnected' || data.db === 'offline';
      if (dbDown) {
        console.error(MYSQL_OFFLINE_MSG);
        setDbOfflineUI(true);
      }
    } catch (_) {
      /* backend not running — handled elsewhere */
    }
  }
